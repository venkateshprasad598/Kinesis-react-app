...
export const startMaster = async () => {
  ...
  const iceServers = [];
  iceServers.push({
    urls: `stun:stun.kinesisvideo.ap-southeast-1.amazonaws.com:443`,
  });
  ...
  signalingClient.on("sdpOffer", async (offer, remotClientId) => {
    ...
    if (!masterState.peerConnectionStatsInterval) {
      ...
      masterState.peerConnectionStatsInterval = setInterval(
        () =>
          peerConnection
            .getStats()
            .then((stats) => console.log("Peer Connection Stats")),
        1000
      );
    }
    ...
    peerConnection.addEventListener("icecandidate", ({ candidate }) => {
      if (candidate) {
        ...
        if (trickleIce) {
          ...
          console.log(
            "All ICE candidates have been generated for client".remotClientId
          );
          if (!trickleIce) {
...
export const createSingallingChannel = async (channelName) => {
  const createSignalChannelResponse = await kinesisVideoClient
